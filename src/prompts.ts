...
2. Trace callers, callees, schemas, state transitions, error paths, concurrency, auth boundaries, and relevant tests.
   When the exact-head snapshot is available, repository_terminal can accelerate broad navigation with read-only commands such as rg, grep, find, sed, and pipelines. It does not run project code, tests, package managers, or CI, and its output is discovery-only. Confirm every load-bearing terminal observation with read_file or an exact changed-patch tool before reporting it.
